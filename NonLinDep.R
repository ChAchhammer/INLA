#The non-linear dependence example
...
set.seed(2)
mwn2 <- 5  + (ll11^2)/6 + rnorm(10000, mean = 0, sd = 0.1) #mean plus spatially structured effect of first process plus white noise
...
sys.time.NonLinDep <- system.time(res.nonlin <- inla(y ~ 0 + b1 + b2 + f(i, model=spde1) + f(k, model = "rw2", fixed=FALSE) + f(j, model=spde2), 
...
#hpd intervals
ms1=res.nonlin$marginals.hyperpar$`Stdev for i`
hpds1 <- inla.hpdmarginal(0.95, ms1)

mr1=res.nonlin$marginals.hyperpar$`Range for i`
hpdr1 <- inla.hpdmarginal(0.95, mr1)

ms2=res.nonlin$marginals.hyperpar$`Stdev for j`
hpds2 <- inla.hpdmarginal(0.95, ms2)

mr2=res.nonlin$marginals.hyperpar$`Range for j`
hpdr2 <- inla.hpdmarginal(0.95, mr2)

#plots for Process 1
#par(mfrow=c(1,3), mar=c(3,3,1,0.3), mgp=c(2,1,0)) 
par(mfrow=c(1,1), mai=c(1,1,1,1))
plot(res.nonlin$marginals.fix[[1]], type='l', mgp=c(2.2,0.6,0), xlim=c(4,8.5),
     xlab=expression(beta[1]), ylab='Density')
abline(v=mean(mwn1), col=2)
abline(v=res.nonlin$summary.fixed$`0.025quant`[1], lty=2)
abline(v=res.nonlin$summary.fixed$`0.975quant`[1], lty=2)

plot(res.nonlin$marginals.hyperpar[[2]], type='l', xlim = c(0.2,2.5), mgp=c(2.1,0.6,0), 
     xlab=expression(sigma[f[1]]^2), ylab='Density')
abline(v=sigma2x1, col=2)
abline(v=hpds1[,1], lty=2)
abline(v=hpds1[,2], lty=2)

plot(res.nonlin$marginals.hyperpar[[1]], type='l', xlim = c(0.2,3.5), mgp=c(1.8,0.6,0),
...
#plots for Process 2
#par(mfrow=c(1,3), mar=c(3,3,1,0.3), mgp=c(2,1,0)) 
plot(res.nonlin$marginals.fix[[2]], type='l',  mgp=c(2,0.6,0), xlim=c(1,6.5),
     xlab=expression(beta[2]), ylab='Density')
abline(v=mean(mwn2), col=2)
abline(v=res.nonlin$summary.fixed$`0.025quant`[2], lty=2)
abline(v=res.nonlin$summary.fixed$`0.975quant`[2], lty=2)

plot(res.nonlin$marginals.hyperpar[[5]], type='l', xlim = c(0.3,2.5), mgp=c(2.1,0.6,0), 
     xlab=expression(sigma[f[2]]^2), ylab='Density')
abline(v=sigma2x2, col=2)
abline(v=hpds2[,1], lty=2)
abline(v=hpds2[,2], lty=2)

plot(res.nonlin$marginals.hyperpar[[4]], type='l', xlim = c(0,1.5), mgp=c(1.8,0.6,0),
...
prd0.m <- inla.mesh.project(pgrid0, (mean(mwn2) + res.nonlin$summary.ran$j$mean))
par(mfrow=c(1,1))
