#The dependence on another latent field example
...
#plotting
image.plot(list(x=Lam11$yrow, y=Lam11$xcol, z=ll11), main='Spatially structured effect', asp=1)
...
set.seed(2)
mwn2 <- 5  + log(ll12)/4 +  rnorm(10000, mean = 0, sd = 0.1) #mean plus spatially structured effect of first process plus white noise
...
sys.time.LatProc <- system.time(res.latproc <- inla(y ~ 0 + b1 + b2 + f(i, model=spde1) + f(k, model = "rw1", fixed=FALSE) + f(j, model=spde2), 
...
#hpd intervals
ms1=res.latproc$marginals.hyperpar$`Stdev for i`
hpds1 <- inla.hpdmarginal(0.95, ms1)

mr1=res.latproc$marginals.hyperpar$`Range for i`
hpdr1 <- inla.hpdmarginal(0.95, mr1)

ms2=res.latproc$marginals.hyperpar$`Stdev for j`
hpds2 <- inla.hpdmarginal(0.95, ms2)

mr2=res.latproc$marginals.hyperpar$`Range for j`
hpdr2 <- inla.hpdmarginal(0.95, mr2)

#plots for Process 1
#par(mfrow=c(1,3), mar=c(3,3,1,0.3), mgp=c(2,1,0)) 
par(mfrow=c(1,1), mai=c(1,1,1,1))
plot(res.latproc$marginals.fix[[1]], type='l', mgp=c(2.2,0.6,0), xlim=c(4,8.5),
     xlab=expression(beta[1]), ylab='Density')
abline(v=mean(mwn1), col=2)
abline(v=res.latproc$summary.fixed$`0.025quant`[1], lty=2)
abline(v=res.latproc$summary.fixed$`0.975quant`[1], lty=2)

plot(res.latproc$marginals.hyperpar[[2]], type='l', xlim = c(0.2,2.5), mgp=c(2.1,0.6,0), 
     xlab=expression(sigma[f[1]]^2), ylab='Density')
abline(v=sigma2x1, col=2)
abline(v=hpds1[,1], lty=2)
abline(v=hpds1[,2], lty=2)

plot(res.latproc$marginals.hyperpar[[1]], type='l', xlim = c(0.2,3.5), mgp=c(1.8,0.6,0),
...
#plots for Process 2
#par(mfrow=c(1,3), mar=c(3,3,1,0.3), mgp=c(2,1,0)) 
plot(res.latproc$marginals.fix[[2]], type='l',  mgp=c(2,0.6,0), xlim=c(3.5,6.5),
     xlab=expression(beta[2]), ylab='Density')
abline(v=mean(mwn2), col=2)
abline(v=res.latproc$summary.fixed$`0.025quant`[2], lty=2)
abline(v=res.latproc$summary.fixed$`0.975quant`[2], lty=2)

plot(res.latproc$marginals.hyperpar[[5]], type='l', xlim = c(0.2,2.5), mgp=c(2.1,0.6,0), 
     xlab=expression(sigma[f[2]]^2), ylab='Density')
abline(v=sigma2x2, col=2)
abline(v=hpds2[,1], lty=2)
abline(v=hpds2[,2], lty=2)

plot(res.latproc$marginals.hyperpar[[4]], type='l', xlim = c(0,2), mgp=c(1.8,0.6,0),
     xlab=expression(rho[2]), ylab='Density')
abline(v=sqrt(8*1)/kappa2, col=2)
abline(v=hpdr2[,1], lty=2)
abline(v=hpdr2[,2], lty=2)

plot(res.latproc$marginals.hyperpar[[3]], type='l', xlim = c(-1.5,5), mgp=c(1.8,0.6,0),
     xlab=expression(beta[3]), ylab='Density')
abline(v=beta3, col=2)
abline(v=res.latproc$summary.hyper$`0.025quant`[5], lty=2)
abline(v=res.latproc$summary.hyper$`0.975quant`[5], lty=2)


pgrid0 <- inla.mesh.projector(mesh, xlim=0:1, ylim=0:1, dims=c(101,101))
prd0.m <- inla.mesh.project(pgrid0, (mean(mwn2) + res.latproc$summary.ran$j$mean))
x11()
